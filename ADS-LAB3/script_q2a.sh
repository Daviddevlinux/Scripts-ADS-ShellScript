...
for TAMANHO_ENTRADA in "${TAMANHOS_ENTRADA[@]}"; do
    for ALGORITMO in "${ALGORITMOS[@]}"; do
        ...
        echo "Executando $N medições para $ALGORITMO com tamanho de entrada $TAMANHO_ENTRADA e valor máximo $VALOR_MAXIMO"
        for i in $(seq 1 $N); do
            echo "Medição $i para $ALGORITMO com tamanho de entrada $TAMANHO_ENTRADA..."
           
            java -cp "bin:." MedidorDeOrdenacao $ALGORITMO $TAMANHO_ENTRADA $VALOR_MAXIMO >> "$ALGORITMO_DIR/medicao_$i.txt"

        done
        echo "Resultados para $ALGORITMO com tamanho de entrada $TAMANHO_ENTRADA salvos no diretório $ALGORITMO_DIR"
    done
done
